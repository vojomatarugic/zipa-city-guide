              ...
              <Link key={event.id} to={`/events/${event.id}`} className="cursor-pointer hover:scale-[1.02] transition-all duration-300 block" style={{ textDecoration: "none" }}>
                ...
                <div className="p-4">
                  <div className="flex items-center gap-2 mb-2 flex-wrap">
                    <span className="text-xs font-medium px-2 py-1 rounded" style={{ background: "#F3F4F6", color: "#00897B" }}>
                      {event.event_type ? eventService.translateEventType(event.event_type, language) : (language === "sr" ? "Bioskop" : "Cinema")}
                    </span>
                    {event.price && (
